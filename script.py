...
if username == "ERR_NOT_SPECIFIED":
    ...
    while True:
        ...
        if new_username.lower() in ["null", "undefined", "None", "ERR_NOT_SPECIFIED"]:
            animated_print(f"Sorry, but usernames can't be {colorama.Fore.BLUE + new_username + colorama.Fore.RESET}. Try a different one.", delay)
        else:
            animated_print(f"Your brand new username is set to {colorama.Fore.BLUE + new_username + colorama.Fore.RESET}. If you want it changed, you can always change your username via ?settings.", delay)
...
You have purchased {shop_upgrades['shopUpgradesPurchased']} shop upgrades.""", delay)
        case "?settings":
            ...
            while True:
                ...
                match settings_input:
                    case "1":
                        ...
                        animated_print(f"Set the setting \"Use Animations\" to {colorama.Fore.GREEN if settings['useAnimation'] else colorama.Fore.RED}{settings['useAnimation']}{colorama.Fore.RESET}", delay)
                    case "2":
                        settings['autoSaveEveryCommand'] = not (settings['autoSaveEveryCommand'])
                        animated_print(f"Set the setting \"Autosave Every Command\" to {colorama.Fore.GREEN if settings['autoSaveEveryCommand'] else colorama.Fore.RED}{settings['autoSaveEveryCommand']}{colorama.Fore.RESET}", delay)
                    case "3":
                        while True:
                            new_username = animated_input("Enter your new username!\n> ", delay)
                            if new_username.lower() in ["null", "undefined", "None", "ERR_NOT_SPECIFIED"]:
                                ...
                                animated_print(f"Your username is now set to {colorama.Fore.BLUE + new_username + colorama.Fore.RESET}.", delay)
                                username = new_username
                                break
                    case "4":
        ...
        case "?rand" | "?random" | "?roll":
            while True:
                ...
                if minimum_number > maximum_number:
                    animated_print(colorama.Style.DIM + "You silly! The minimum number can't be larger can the maximum number." + colorama.Style.RESET_ALL, delay)
                    continue
                if minimum_number == maximum_number:
                    animated_print(colorama.Style.DIM + "You silly! Both numbers can't be the same, otherwise it would always return the number and breaks the entire concept of random!" + colorama.Style.RESET_ALL, delay)
                    continue
                break
            animated_print(f"You picked {colorama.Style.BRIGHT}{minimum_number}{colorama.Style.RESET_ALL} as your minimum number, and {colorama.Style.BRIGHT}{maximum_number}{colorama.Style.RESET_ALL} as your maxmimum number. Your result is {colorama.Style.BRIGHT}{random.randint(minimum_number, maximum_number)}{colorama.Style.RESET_ALL}.", delay)
        case "?date":
            animated_print(f"Your current date is {colorama.Style.BRIGHT + time.strftime(r"%B %d (%A), %Y") + colorama.Style.RESET_ALL}.", delay)
        case "?time":
            animated_print(f"Your current time is {colorama.Style.BRIGHT + time.strftime(r"%X") + colorama.Style.RESET_ALL}.", delay)
        case "?datetime" | "?timedate":
            animated_print(f"Your current date & time is {colorama.Style.BRIGHT + time.strftime(r"%B %d (%A), %Y | %X") + colorama.Style.RESET_ALL}.", delay)
        ...
        case "?golt" | "?greaterorlowerthan":
            ...
            while True:
                ...
                if choice < user_choice and user_choice <= 100:
                    ...
                    attempts += 1
                    animated_print(f"You've guessed it right! You used {attempts} attempts in this game, and the answer was {colorama.Style.BRIGHT}{choice}{colorama.Style.RESET_ALL}.", delay)
                    break
                attempts += 1
...
animated_print(f"Goodbye {colorama.Fore.BLUE + username + colorama.Fore.RESET}, see you again!", delay)
